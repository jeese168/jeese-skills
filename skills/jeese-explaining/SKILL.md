...
优先用明确的对象名称说明动作和关系。代词是否清楚，要结合当前完整句段及前文仍在讨论的对象判断。读者能直接确定唯一所指时，自然使用代词；段落中途出现多个可能所指、插入其他对象或切换主体时，即使只隔逗号或分号，也用简短名称重新点明对象。较长句子按动作关系合理断句，并在主体切换处写明由谁执行。跨段承接时同样点明所指对象。引用前文时，点明具体概念、规则或结论，就近带出理解当前句子所需的条件，让读者在当前位置接得上解释。
...
详细方法见 [概念解释](references/concepts.md) 与 [文字组织](references/organization.md)。标准、快速和源码带读按各自情况读取；针对性解答和现实判断在出现概念缺口或组织需要时补读。本节的共同讲法始终适用，详细文件用于补足当前需要的方法和例子。
