...
STAGES = {"learning": 4, "proposal": 4, "maintenance": 3, "merge": 3}
FIELDS = {"run", "flow", "stage", "diagram"}
...
def validate_metadata(meta, line):
    ...
    if not isinstance(meta["flow"], str) or meta["flow"] not in STAGES:
        raise InvalidArtifact(f"{prefix}：flow 必须是 learning、proposal、maintenance 或 merge")
    stage = meta["stage"]
    if type(stage) is not int or not 1 <= stage <= STAGES[meta["flow"]]:
...
def select_blocks(text, run, flow, stages):
    selected = {}
    errors = []
    for meta, body, line in parse_blocks(text):
        if meta["run"] != run:
            continue
        if meta["flow"] != flow:
            errors.append(f"第 {line} 行：run={run} 的执行路径与本次命令不一致")
        stage = meta["stage"]
...
def check(text, run, flow, through):
    stages = range(1, through + 1)
    selected, errors = select_blocks(text, run, flow, stages)
    for stage in stages:
        if stage not in selected:
            continue
...
def main():
    ...
    parser.add_argument("--run", required=True, help="本轮文档任务执行的唯一标识")
    parser.add_argument("--flow", required=True, choices=STAGES)
    ...
    if not args.run.strip() or any(not 1 <= stage <= STAGES[args.flow] for stage in stages):
    ...
    try:
        if args.read_stages is not None:
            ...
            selected, errors = select_blocks(text, args.run, args.flow, stages)
        else:
            errors = check(text, args.run, args.flow, args.through)
    ...
    if args.read_stages is not None:
        ...
        for stage in stages:
            ...
            print(END)
        return 0
    print(f"结构检查通过：{args.run} / {args.flow}，阶段 1–{args.through}。")
    print("仅确认标记、非空区块和声明的图代码块；不证明内容正确、表达充分或实际执行顺序。")
    return 0
