...
def block(stage, body=FACT, run="read-1", flow="learning", diagram=False):
    meta = dict(run=run, flow=flow, stage=stage, diagram=diagram)
...
class ArtifactChecks(unittest.TestCase):
    ...
    def invoke(self, text, through=1, flow="learning", run="read-1", file="work.md",
               read_stages=None):
        path = self.task / file
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(text, encoding="utf-8")
        result = self.command(through, flow, run, file, read_stages)
        self.assertEqual(path.read_text(encoding="utf-8"), text, "validator must not write artifacts")
        return result

    def command(self, through=1, flow="learning", run="read-1", file="work.md",
                read_stages=None):
    ...
    def test_each_flow_accepts_only_completed_prefix(self):
        for flow, count in (("learning", 4), ("proposal", 4), ("maintenance", 3), ("merge", 3)):
            for through in range(1, count + 1):
                with self.subTest(flow=flow, through=through):
                    text = "任务：为当前修改理解读取路径。\n" + "".join(
                        block(i, flow=flow) for i in range(1, through + 1))
                    self.assert_status(0, self.invoke(text, through, flow))
    ...
    def test_old_round_cannot_fill_new_round(self):
        old = "".join(block(i, flow="proposal", run="part-1") for i in range(1, 5))
        new = block(1, "本轮维护说明：补充缓存失效后重新读取的条件。", flow="proposal", run="part-2")
        self.assert_status(0, self.invoke(old + new, flow="proposal", run="part-2"))
        self.assert_status(1, self.invoke(old + new, through=2, flow="proposal", run="part-2"))
        self.assert_status(0, self.invoke(old + new + block(2, flow="proposal", run="part-2"),
                                         through=2, flow="proposal", run="part-2"))

    def test_same_run_cannot_mix_execution_paths(self):
        text = block(1) + block(1, flow="proposal", run="proposal-1")
        self.assert_status(0, self.invoke(text, flow="proposal", run="proposal-1"))
        self.assert_status(1, self.invoke(text, run="proposal-1"))
        self.assert_status(1, self.invoke(text, flow="proposal", run="read-1"))
    ...
    def test_duplicates_malformed_metadata_and_unclosed_blocks_fail(self):
        ...
        for text in (valid + valid, valid.replace('"stage": 1', '"stage": true'),
                     ...
                     valid.replace('"flow": "learning"', '"flow": []'),
    ...
    def test_bad_path_missing_file_and_invalid_cli_are_errors(self):
        ...
        self.assert_status(2, self.command(flow="unknown"))
        self.assert_status(2, self.command(flow="maintenance", through=4))
    ...
    def test_read_missing_conflicting_or_duplicate_targets_emits_no_partial_content(self):
        for text in (block(1), block(1) + block(2, flow="maintenance"),
    ...
    def test_invalid_read_arguments(self):
        ...
        result = subprocess.run(
            [sys.executable, str(SCRIPT), "--task-dir", str(self.task), "--run", "read-1",
             "--flow", "learning", "--through", "1", "--read-stages", "1"],
            text=True, capture_output=True, check=False,
        )
        self.assert_status(2, result)
