...
入口与各 reference 中的路由、交接和处置说明，供当前执行任务的主 Agent 使用。情况二和情况六生成对外指令时，围绕接收方的任务写入对象、背景、依据、检查重点、报告方式和实际权限；后续核实、分类和流转留在主 Agent 自己的指导中。
...
**双窗口送审的交接。** 情况二生成的指令完整说明审查任务和回复要求，用户自行拼接对象后交给审查窗口执行。若用户仍在完整指令旁点名本 Skill，按已经明确的指令执行；这不改变情况三的独立文档核查用途。同一审查窗口的后续复查沿用已收到的指令，并以用户的新要求和新材料为准。

**只有点名。** 用户就是发了 `/jeese-engineers` 或同等点名，同一条里没有指定、也没有上面那些材料：停下来问这一轮要交什么。根据当前窗口进度，选取最相关的选项来询问或推荐。例如刚改完代码、路径还没交，推荐先做情况一；路径已经有了、审查指令还没有，推荐情况二。推荐之后等用户应一声再做。
...
产物是经过核实修正的中间材料及简短处置记录。完成后继续调用方原来的工作流，最终讲解由该工作流交付。主 Agent 负责传入完整审查任务并接收发现，子 Agent 直接完成本次只读检查；保持这一层调度，不让子 Agent 继续派出下一层。
