...
当前的渐进式披露只决定 Agent 在什么时候读取 `SKILL.md` 和对应 references。这些规则进入当前任务的上下文后会持续约束后续行为，但不会自动要求 Agent 每读完一层就留下一份中间结果。

如果长期真实使用显示同类约束反复被遗漏，或者模型更新后遵循性明显下降，再考虑把受影响的 Skill 改成带检查点的工作流：先形成任务对象、读者、目标和材料来源的判断，再根据核心规则形成信息组织决定，最后加载题材和格式规则完成产物。检查点只保存后续任务需要的前提和可审核决定，不要求展示模型的完整内部推理。没有稳定问题之前，不启用这种更重的形态。
...
讲解产生的定义、因果链、例子和边界可以成为后续写作的原始材料。这些 Skill 彼此没有运行时依赖，也不要求同时激活；`jeese-engineers` 只在明确调用时启用。运行所需材料都收拢在各自的 Skill 目录内，下一阶段会通过真实任务继续核对它们的触发边界。
